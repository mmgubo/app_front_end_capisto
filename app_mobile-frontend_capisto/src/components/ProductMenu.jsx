    ...
    <div id='product' className='flex flex-col items-center gap-4 py-16 text-gray-800'>
        ...
        <div className='flex sm:justify-center gap-4 pt-5 w-full overflow-scroll'>
            {specialityData.map((item,index)=>(
                <Link onClick={()=>{ navigate('/appointments'); scrollTo(0,0)}} className='flex flex-col items-center text-xs cursor-pointer flex-shrink-0 hover:translate-y-[-10px] transition-all duration-500' key={index} to={'/appointments/${item.id}'}>
